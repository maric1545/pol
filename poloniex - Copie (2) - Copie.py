    ...
    for n in range (len(x)):
        if p == x[n]['date']:
            ...
            if Date == "11/09/2017 01:45:00":
                ...
                value = (close - EMA_100(pre)) * 2/13 + EMA_100(pre)
                dic_100[Date] = value
                return value"""
import datetime
...
startTime = str (time.mktime(datetime.datetime.strptime(startdate, "%Y-%m-%d %H:%M:%S")
                            .timetuple()))
period = str (300)
pair = "BTC_VTC"
...
while True:
    ...
    def EMA(date):
        ...
        if date in dic_EMA:
            return dic_EMA[date]
        z = (close - EMA(x[-3]["date"])) * 2/51 + EMA(x[-3]["date"])
        dic_EMA[date] =  z
